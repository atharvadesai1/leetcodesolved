...
def fairCandySwap(aliceSizes, bobSizes):
        ...
        target_sum = (alice_candies + bob_candies) // 2
          
        alice_set = set(aliceSizes)
        for bob_candy in bobSizes:
            # If Bob swaps a candy of size x, Alice must receive a candy of size target_sum - (bob_candies - x)
            alice_candy = target_sum - (bob_candies - bob_candy)
